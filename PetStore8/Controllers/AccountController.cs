...
namespace PetStore8.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Register(CustomerRegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                var user = new ApplicationUser { UserName = model.Email, Email = model.Email, PhoneNumber = model.Phone, FirstName = model.FirstName, LastName = model.LastName, Address = model.Address };
                var result = await UserManager.CreateAsync(user, model.Password);
                if (result.Succeeded)
                {
                    //Luu thong tin khach hang vao table Customer
        ...
        // GET: /Account/RegisterAccounts
        //[Authorize(Roles = "Admin")]
        [AllowAnonymous]
        public async Task<ActionResult> RegisterAccounts()
        {
            //Them 2 nhan vien
            for (int i = 1; i < 3; i++)
            {                
                ...
                {
                    UserName = $"e{i}@gmail.com",
                    Email = $"e{i}@gmail.com",
                    PhoneNumber = $"e{i}12345678",
                    FirstName = $"e{i}First Name",
                    LastName = $"e{i}Last Name",
                    Address = $"e{i}Address"
                };
            ...
            {                
                var user = new ApplicationUser
                {
                    UserName = $"c{i}@gmail.com",
                    Email = $"c{i}@gmail.com",
                    PhoneNumber = $"c{i}12345678",
                    FirstName = $"c{i}First Name",
                    LastName = $"c{i}Last Name",
                    Address = $"c{i}Address"
                };
                var result = await UserManager.CreateAsync(user, "1");
                if (result.Succeeded)
        ...
        {
            List<string> positions = new List<string> { "CEO", "Director", "Deputy Director", "Head of Department", "Deputy of Department", "Staff", "Trainee" };
            ViewBag.positions = new SelectList(positions, "Staff");
